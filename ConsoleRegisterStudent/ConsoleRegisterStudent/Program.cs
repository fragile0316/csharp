...
namespace ConsoleRegisterStudent
{
    class Program
    {
            ...
            do
            {
                ...
                switch (ValidateChoice(choice, firstChoice, secondChoice, thirdChoice, totalCredit))
                {
                    case -1:
                        ...
                        break;
                    case -2:
                        Console.WriteLine("You have already registered for this {0} course.", ChoiceToCourse(choice)); // previously registerd
                        break;
                    case -3:
        ...
        {
            if (choice < 1 || choice > 7) // previously choice < 1 || choice > 70
                return -1;
            else if (choice == firstChoice || choice == secondChoice || choice == thirdChoice) // previously (... && ... && ...)
                return -2;
            else if (totalCredit >= 9) // previously totalCredit > 9
                return -3;
            return 0; // previously -4
        }
